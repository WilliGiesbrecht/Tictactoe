...
function fillShape(id) {
    if (!fields[id] && !gameOver) {
        if (currentShape == 'cross') {
            currentShape = 'circle';
            document.getElementById('player-two').classList.remove('playerInactive');
            document.getElementById('player-one').classList.add('playerInactive');
        } else {
            currentShape = 'cross';
            document.getElementById('player-one').classList.remove('playerInactive');
            document.getElementById('player-two').classList.add('playerInactive');
        }
...
function restart() {
    ...
    for (let i = 1; i < 9; i++) {
        document.getElementById('line-' + i).style.transform = 'scale(0.0)';
